...
export default function PatternCard({ pattern, isExpanded, onToggle }: {
  pattern: Pattern
  isExpanded: boolean
  onToggle: () => void
}) {
      ...
      {isExpanded && (
        <div className="space-y-4 mt-4 pt-4 border-t border-white/10">
          ...
          <div>
            ...
            <ol className="space-y-1.5">
              {pattern.sequence.map((step, i) => (
                <li key={`${pattern.id}-step-${i}`} className="flex items-start gap-2 text-sm text-gray-400">
          ...
          <div className="bg-black/30 rounded-lg p-4">
            ...
            <p className={`text-xs ${colorClasses.text} mt-2`}>&mdash; {pattern.quote.speaker}</p>
          </div>
